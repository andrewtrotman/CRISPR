...
/*
        ...
        -------------------------
    */
void compute_intersection_list(std::string test_guide, const uint64_t *key, size_t key_length, const uint64_t *data, size_t data_length, std::vector<uint64_t> &matches, std::vector<size_t> &positions)
        {
        matches.push_back(pack20mer(test_guide.c_str()));
...
//fstd::cout << matches.size() << " ";
    ...
    */
    std::vector<uint64_t> load_guides(const std::string &filename)
        ...
        void linearCongruential(
            std::vector<std::string>& selected,
            const std::vector<uint64_t>& guides)
        {
            unsigned long long int seed = 17;
            unsigned long long int m = guides.size()-1; // Modulus parameter
            unsigned long long int a = 3; // Multiplier term
            unsigned long long int c = 3; // Increment term
            ...
            for (int i = 0; i < selected.size(); i++) {
                ...
                selected[i] = unpack20mer(guides[next]);
                next = ((next * a)+c) % m;
            }
        }
...
//        std::sort(variations.begin() + 1, variations.end());        // +1 because the first element is the test guide
        ...
        fast_binary_search::compute_intersection_list(test_guides[i],variations.data(), variations.size() - 1, packed_genome_guides.data(), packed_genome_guides.size(), matches, positions);
...
//    std::string guides_filename = "humanGenomeGuides.txt";
    ...
    */
    ...
    if (true) {
        ...
        fast_binary_search::linearCongruential(test_guides, packed_genome_guides);
//        fast_binary_search::select_random_vectors(test_guides, packed_genome_guides);
        ...
        for (int which=0; which<TESTSIZE; which++) {
...
//    process_chunk(start_index, test_guides.size(), test_guides, packed_genome_guides);
    ...
    */
    ...
    for (int i=0; i<all_matches.size();i++) {
        size_t numMatches = all_matches[i].size()-1;
        sum_matches += numMatches;
        if (numMatches>0)
            ++empty_count;
        if (numMatches > max_matches)
            max_matches = numMatches;
        if (numMatches < min_matches)
            min_matches = numMatches;
        if ((min_matches == max_matches) && i>0 ) {
            std::cout << "OOPS! min = " << min_matches << "max = " << max_matches << ", i = " << i << std::endl;
        }
//        std::cout << all_matches[i].size() << std::endl;
//        for (int j=0; j<all_matches[i].size(); j++) {
//            std::cout << fast_binary_search::unpack20mer(all_matches[i][j]) << "  ";
//            std::cout << all_positions[i][j] << std::endl;
//        }
//        std::cout << std::endl;
    }
